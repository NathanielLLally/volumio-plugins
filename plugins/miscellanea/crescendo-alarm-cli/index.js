...
AlarmClock.prototype.getUIConfig = function()
{
	...
	//enable
	uiconf.sections[0].content[1].value=config.get('crescendo');
...
AlarmClock.prototype.setVolume = function(alarm) {
  let self = this
  let vol = alarm.volume || 10
  self.logger.info( "setVolume " +" volumiosetvolume: "+ vol );
...
AlarmClock.prototype.applyConf = function(conf) {
  ...
  for (var i in conf) {
    ...
    if (item.enabled) {
      item.volume_start = 15
      item.volume_end = 40
      item.volume = item.volume_start
//      item.crescendo_time = 900
      item.crescendo_time = 420


      var vf = self.setVolume.bind(self);

      var func = self.fireAlarm.bind(self);


      self.logger.info( "Alarm: scheduling alarm at" +moment(item.time).format()+" at volume from "+ item.volume_start + " to "+item.volume_end );
      ...
      let vspan = item.volume_end - item.volume_start
      let tick_time = item.crescendo_time / vspan


      var then = new moment(item.time);

      //oddly enough, volumio will set the exact time when the add button was clicked
      //scheduler sets off on the minute
      //thus we compensate
      //
      then.seconds(0)
      for (let tick = item.volume_start+1; tick <= item.volume_end; tick++) {

        then.add(tick_time, 'seconds')
        let r = new schedule.RecurrenceRule();
        r.hour = then.hours();
        r.minute = then.minutes();
        r.second = then.seconds();

        let clone = {...item}
        clone.volume = tick
        self.logger.info( "Alarm: scheduling volset to "+tick+" at " +then.format() );
        var k = schedule.scheduleJob(r, function(){
          vf(clone);
        });
        self.jobs.push(k);
      }
